...
const AlgorithmDescription = () => {
    ...
    const descriptions = {
        ...
        depthFirstSearch: "Depth-First Search (DFS) is an algorithm that explores a graph by traversing as deep as possible along each branch before backtracking. When Aragorn, Legolas, and Gimli were pursuing the Uruk-hai - following each trail to its end before backtracking to try another path.",
        breadthFirstSearch: "Breadth-First Search (BFS) is an algorithm that explores a graph level by level, starting from the root node and visiting all of its neighbors before moving on. Spreads like the lighting of the beacons of Gondor - the signal spreading to all nearby beacons first before reaching the more distant ones.",
        defaultAlgorithm: "No specific algorithm selected. Use the navigation options to explore algorithms."
    };
